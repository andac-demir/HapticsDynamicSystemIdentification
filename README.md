# Dynamic System Identification Modeling for Haptic Interactions in VR/AR Environments
------------------------------------
...
We train the first one of these dynamic models in time domain, the second one in multiresolution sparse wavelet domain and the third one in hybrid time-wavelet domain. Our procedure to train a dynamic model in time domain can be divided into two stages. In the first stage, we solve for the maximum likelihood (ML) parameter vector of a parsimonious integrated vector autoregression model (VAR) and estimate the latency between endogenous time variables, utilizing a grid search over the log likelihood scores. In the second stage, we explore the modality dependencies between synchronized EEG, EMG and haptic interactions by training VAR models of the same structure. We use our knowledge of signal latency, lag orders and modality dependencies to predict EEG and haptic forces from any provided different combination of EEG, EMG and force measurements.  

In wavelet domain dynamic model, we first translate signals from time domain by multi-level fast wavelet transformations (FWT) into frequency domain using 4-tap Daubechies mother wavelets. The obtained coefficients are filtered by soft thresholding, which allows near-lossless compression. Then we solve for the ML parameter vector of the same VAR model, using a sparse representation of wavelet coefficients. 

The hybrid dynamic model is a mixed input system that integrates the time samples with sparse wavelet coefficients from different resolution levels. We benchmark the performance of these three dynamic models against each other across each participant and discuss the potential how these models can guide external stimulation parameters for haptic interaction simulation in scenarios, including teleoperations and virtual environments.
